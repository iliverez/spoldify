...
public final class PlayableContentFeeder {
    ...
    private @NotNull LoadedStream loadTrack(@NotNull TrackId id, @NotNull AudioQualityPicker audioQualityPicker, boolean preload, @Nullable HaltListener haltListener) throws IOException, MercuryClient.MercuryException, ContentRestrictedException, CdnManager.CdnException {
        ...
        if (track == null) {
            track = getTrackViaExtendedMetadata(id);
            if (track != null) {
                LOGGER.info("Got track files via extended metadata API: {} files", track.getFileCount());
            }
        }
    ...
    @Nullable
    private Metadata.Track getTrackViaExtendedMetadata(@NotNull TrackId id) {
        try {
            ExtendedMetadata.BatchedEntityRequest request = ExtendedMetadata.BatchedEntityRequest.newBuilder()
                .addEntityRequest(
                    ExtendedMetadata.EntityRequest.newBuilder()
                        .setEntityUri(id.toSpotifyUri())
                        .addQuery(
            ...
            ExtendedMetadata.BatchedExtensionResponse extMeta = session.api().getExtendedMetadata(request);
            for (ExtendedMetadata.EntityExtensionDataArray entry : extMeta.getExtendedMetadataList()) {
                for (EntityExtensionDataOuterClass.EntityExtensionData data : entry.getExtensionDataList()) {
                    if (data.hasExtensionData()) {
                        Metadata.Track extTrack = Metadata.Track.parseFrom(data.getExtensionData().getValue());
                        if (extTrack.getFileCount() > 0) {
                            return extTrack;
                        }
                        Metadata.Track alt = pickAlternativeIfNecessary(extTrack);
                        if (alt != null) return alt;
                    }
                }
            }
        } catch (Exception ex) {
            LOGGER.warn("Failed to get track via extended metadata API", ex);
        }
        return null;
    }
